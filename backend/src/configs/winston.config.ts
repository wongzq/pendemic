...
const formatMessage = (
  ...
  return `${req.method} ${req?.originalUrl} ${type} ${message}`;
};
...
type LoggerType = {
  ...
  success: (req: Request, ...msg: any) => winston.Logger;
  failure: {
    warn: (req: Request, ...msg: any) => winston.Logger;
    error: (req: Request, ...msg: any) => winston.Logger;
  };
  file: string;
};
